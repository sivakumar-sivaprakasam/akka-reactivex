...
object Async {
  ...
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = {
    eventuallyX.map(value => value % 2 == 0)
  }
  ...
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] = {
    eventuallyX.map(value => value).recover { case exception: Exception => -1 }
  }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    var result: Future[A] = makeAsyncComputation()
    result.map(value => value).recoverWith {
      case e => {
        for (x <- 2 to maxAttempts)  {
          result = makeAsyncComputation()
        }
        result
      }
    }
