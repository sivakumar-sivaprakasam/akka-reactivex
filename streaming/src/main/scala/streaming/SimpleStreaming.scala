 ...
 */
object SimpleStreaming extends ExtraStreamOps {

//  def main(args: Array[String]): Unit = {
//    implicit val system = ActorSystem()
//    val ints: Source[Int, NotUsed] = Source.fromIterator(() => Iterator.from(1)).take(10) // safety limit, to avoid infinite ones
//    val tenStrings = SimpleStreaming.mapToStrings(ints)
//    tenStrings.map(println(_))
//  }

  /** Change each of the streamed elements to their String values */
  def mapToStrings(ints: Source[Int, NotUsed]): Source[String, NotUsed] = ints.map(x => x.toString)
